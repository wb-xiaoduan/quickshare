...
app.get("/view/:id", async (req, res) => {
    try {
        ...
        // 打印原始内容的前100个字符，帮助调试
        console.log(
            `原始内容前100字符: ${page.html_content.substring(0, 100)}...`
        );
        ...
        if (isPureMermaid) {
            console.log(
                "[DEBUG] 检测到纯 Mermaid 语法，强制设置为 mermaid 类型"
            );
        ...
        // 在渲染内容中添加代码类型信息和标题
        // 使用页面存储的标题，如果没有则使用默认标题
        const pageTitle = page.title || "未命名页面";

        // 判断是否需要修改标题
        let contentWithTypeInfo;
        if (renderedContent.includes("<title>")) {
            // 替换已有的标题
            contentWithTypeInfo = renderedContent.replace(
                /<title[^>]*>([^<]+)<\/title>/i,
                `<title>${pageTitle}</title><meta name="code-type" content="${contentType}">`
            );
        } else {
            // 在head结束前添加标题和元数据
            contentWithTypeInfo = renderedContent.replace(
